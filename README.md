...
- **Strand-Finder** (`posts/straende-sardinien.html`, `en/posts/beaches-sardinia.html`): Filter-Chips über der Strandkarte (Familie/flaches Wasser, Ohne Auto, Reservierung/Ticket, Schnorcheln, Ruhig & wild, Wind- & Kitesurfen). Mehrere Chips gelten gemeinsam (UND); nicht passende Pins und Listeneinträge werden abgeblendet, ein Zähler zeigt die Treffer. Die Kriterien stehen als `data-f`-Attribute an Pins und Listeneinträgen und stammen aus den Einzelartikeln; fehlt ein Kriterium, ist es dort nicht belegt. Neue Angaben in einem Strandartikel bitte auch in `data-f` nachziehen (DE und EN).
...
- **Google Search Console:** Die Datei `googleda72650e443d5a8f.html` im Hauptordner ist der Bestätigungsnachweis der Search Console und muss bleiben. Die Sitemap enthält für jede Seite ein Änderungsdatum (`lastmod`, aus der Git-Historie ohne Sammel-Commits); `forum.html` steht bewusst nicht darin und trägt `noindex`, solange das Forum ausgeblendet ist.
- **Teilen & Suchmaschinen:** Jede Seite hat eine kanonische Adresse (`canonical`), Open-Graph-Angaben (`og:title`, `og:description`, `og:image`, `og:url`, `og:locale` …) und eine Twitter-Card. Als Vorschaubild dient das Titelbild des Artikels, für Startseiten, Forum und Links das Bild `images/og-home.jpg` (1200 × 630 px, Foto aus dem Hero mit Logo und Schriftzug). Artikel haben zusätzlich `BreadcrumbList`-Daten. Diese Blöcke wurden per Skript ergänzt; bei neuen Seiten die Angaben von einer bestehenden Seite übernehmen und Titel, Beschreibung, URL und Bild anpassen.
...
| `images/ichnusa-titel.jpg`, `images/ichnusa-titel-hoch.jpg` | Titelbild Bier-Artikel und Kachel: Ichnusa Anima Sarda auf einem Felsen, Foto: Pansebert, CC0, via Wikimedia Commons (Datei "Ichnusa (Heineken Italia).jpg", zugeschnitten); Marke des Herstellers, redaktionelle Verwendung |
